...
static sgx_status_t SGX_CDECL sgx_ecall_nth_element(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_nth_element_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_nth_element_t* ms = SGX_CAST(ms_ecall_nth_element_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	DATATYPE* _tmp_data_org = ms->ms_data_org;



	ecall_nth_element(_tmp_data_org, ms->ms_dim, ms->ms_k);


	return status;
}

static sgx_status_t SGX_CDECL sgx_ecall_min_heap(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_min_heap_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_min_heap_t* ms = SGX_CAST(ms_ecall_min_heap_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	DATATYPE* _tmp_data_org = ms->ms_data_org;



	ecall_min_heap(_tmp_data_org, ms->ms_dim, ms->ms_k);
...
SGX_EXTERNC const struct {
	...
	2,
	{
		{(void*)(uintptr_t)sgx_ecall_nth_element, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_min_heap, 0, 0},
	}
};
